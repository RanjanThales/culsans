...
class read_miss_write extends dcache_sequence; 
  `uvm_object_utils(read_miss_write) 

    logic [63:0] addr_i;

  function new(string name = "read_miss_write"); 
    super.new(name); 
  endfunction 
  ...
  virtual task body(); 
   ...
   for (int i=0; i<1; i++) begin  
    ...
    //addr_i = ArianeCfg.CachedRegionAddrBase[0];   
    //req.addr_i = 64'h80060000;   
    req.addr = 64'h80060000;   
    
      //  req.addr = addr_i + (i << DCACHE_INDEX_WIDTH);
        req.data_we = 1;
        //req.data = 64'hBEEF_CAF0 + i;
        req.data = 64'hBEEF_CAFE;
       finish_item(req); 
end
  endtask 
endclass 
...
class read_miss_read extends dcache_sequence; 
  `uvm_object_utils(read_miss_read) 

    logic [63:0] addr_i;

  function new(string name = "read_miss_read"); 
    super.new(name); 
  endfunction 
...
class cache_evict_w1_seq extends dcache_sequence; 
  `uvm_object_utils(cache_evict_w1_seq) 
 
  function new(string name = "cache_evict_w1_seq"); 
    super.new(name); 
  endfunction 
...
class cache_evict_w2_seq extends dcache_sequence; 
  `uvm_object_utils(cache_evict_w2_seq) 
 
  function new(string name = "cache_evict_w2_seq"); 
    super.new(name); 
  endfunction 
  ...
  virtual task body(); 
    ...
    start_item(req); 
    req.addr     = 64'h0000000080068000; 
    req.data     = 64'hD00D_CAFE; 
    req.data_we  = 1'b1; 
    finish_item(req); 
...
class cache_evict_r1_seq extends dcache_sequence; 
  `uvm_object_utils(cache_evict_r1_seq) 
...
class cache_evict_r2_seq extends dcache_sequence; 
  `uvm_object_utils(cache_evict_r2_seq) 
