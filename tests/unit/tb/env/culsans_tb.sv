...
//    );
   ...
   // assign i_culsans.gen_ariane[core_idx].i_ariane.i_cva6.WB.i_cache_subsystem.axi_resp_i
...
for (genvar core_idx=0; core_idx<NB_CORES; core_idx++) begin
   ...
   assign i_culsans.core_to_CCU[core_idx].ar_addr=   ace_bus_dv[core_idx].ar_addr;
   assign i_culsans.core_to_CCU[core_idx].ar_id=     ace_bus_dv[core_idx].ar_id;
   assign i_culsans.core_to_CCU[core_idx].ar_len=    ace_bus_dv[core_idx].ar_len;
   assign i_culsans.core_to_CCU[core_idx].ar_size=   ace_bus_dv[core_idx].ar_size;
   assign i_culsans.core_to_CCU[core_idx].ar_burst=  ace_bus_dv[core_idx].ar_burst;
   assign i_culsans.core_to_CCU[core_idx].ar_lock=   ace_bus_dv[core_idx].ar_lock;
   assign i_culsans.core_to_CCU[core_idx].ar_cache=  ace_bus_dv[core_idx].ar_cache;
   assign i_culsans.core_to_CCU[core_idx].ar_prot=   ace_bus_dv[core_idx].ar_prot;
   assign i_culsans.core_to_CCU[core_idx].ar_qos=    ace_bus_dv[core_idx].ar_qos;
   assign i_culsans.core_to_CCU[core_idx].ar_region= ace_bus_dv[core_idx].ar_region;
   assign i_culsans.core_to_CCU[core_idx].ar_user=   ace_bus_dv[core_idx].ar_user;
   assign i_culsans.core_to_CCU[core_idx].ar_valid=  ace_bus_dv[core_idx].ar_valid;
   assign i_culsans.core_to_CCU[core_idx].ar_snoop=  ace_bus_dv[core_idx].ar_snoop;
   assign i_culsans.core_to_CCU[core_idx].ar_bar=    ace_bus_dv[core_idx].ar_bar;
   assign i_culsans.core_to_CCU[core_idx].ar_domain= ace_bus_dv[core_idx].ar_domain;


   assign i_culsans.core_to_CCU[core_idx].r_ready = ace_bus_dv[core_idx].r_ready  ;

   assign i_culsans.core_to_CCU[core_idx].aw_id      = ace_bus_dv[core_idx]. aw_id      ; 
   assign i_culsans.core_to_CCU[core_idx].aw_addr    = ace_bus_dv[core_idx]. aw_addr    ;
   assign i_culsans.core_to_CCU[core_idx].aw_len     = ace_bus_dv[core_idx]. aw_len     ;
   assign i_culsans.core_to_CCU[core_idx].aw_size    = ace_bus_dv[core_idx]. aw_size    ;
   assign i_culsans.core_to_CCU[core_idx].aw_burst   = ace_bus_dv[core_idx]. aw_burst   ;
   assign i_culsans.core_to_CCU[core_idx].aw_lock    = ace_bus_dv[core_idx]. aw_lock    ;
   assign i_culsans.core_to_CCU[core_idx].aw_cache   = ace_bus_dv[core_idx]. aw_cache   ;
   assign i_culsans.core_to_CCU[core_idx].aw_prot    = ace_bus_dv[core_idx]. aw_prot    ;
   assign i_culsans.core_to_CCU[core_idx].aw_qos     = ace_bus_dv[core_idx]. aw_qos     ;
   assign i_culsans.core_to_CCU[core_idx].aw_region  = ace_bus_dv[core_idx]. aw_region  ;
   assign i_culsans.core_to_CCU[core_idx].aw_atop    = ace_bus_dv[core_idx]. aw_atop    ;
   assign i_culsans.core_to_CCU[core_idx].aw_user    = ace_bus_dv[core_idx]. aw_user    ;
   assign i_culsans.core_to_CCU[core_idx].aw_valid   = ace_bus_dv[core_idx]. aw_valid   ;


    //Response signals
   assign ace_bus_dv[core_idx].aw_ready  = i_culsans.i_ccu.slv_ports[core_idx].aw_ready   ;
   assign ace_bus_dv[core_idx].ar_ready  = i_culsans.i_ccu.slv_ports[core_idx].ar_ready   ;
   assign ace_bus_dv[core_idx].w_ready   = i_culsans.i_ccu.slv_ports[core_idx].w_ready    ;
   assign ace_bus_dv[core_idx].r_resp    = i_culsans.i_ccu.slv_ports[core_idx].r_resp     ;
  
   assign ace_bus_dv[core_idx].r_last    = i_culsans.i_ccu.slv_ports[core_idx].r_last     ;
   assign ace_bus_dv[core_idx].r_valid   = i_culsans.i_ccu.slv_ports[core_idx].r_valid    ;
   assign ace_bus_dv[core_idx].b_valid   = i_culsans.i_ccu.slv_ports[core_idx].b_valid    ;
   assign ace_bus_dv[core_idx].b_resp    = i_culsans.i_ccu.slv_ports[core_idx].b_resp     ;


   assign ace_bus_dv[core_idx].r_data   = i_culsans.i_ccu.slv_ports[core_idx].r_data    ;
   //assign i_culsans.core_to_CCU[core_idx].b_valid    = ace_bus_dv[core_idx].b_valid     ;



   //assign i_culsans.core_to_CCU[core_idx].aw_ready   = ace_bus_dv[core_idx]. aw_ready   ;
   assign i_culsans.core_to_CCU[core_idx].aw_snoop   = ace_bus_dv[core_idx]. aw_snoop   ;
   assign i_culsans.core_to_CCU[core_idx].aw_bar     = ace_bus_dv[core_idx]. aw_bar     ;
   assign i_culsans.core_to_CCU[core_idx].aw_domain  = ace_bus_dv[core_idx]. aw_domain  ;
   assign i_culsans.core_to_CCU[core_idx].aw_awunique= ace_bus_dv[core_idx]. aw_awunique;
   
   assign i_culsans.core_to_CCU[core_idx].w_data     = ace_bus_dv[core_idx].w_data   ;
   assign i_culsans.core_to_CCU[core_idx].w_strb     = ace_bus_dv[core_idx].w_strb      ;
   assign i_culsans.core_to_CCU[core_idx].w_user     = ace_bus_dv[core_idx].w_user      ;
   assign i_culsans.core_to_CCU[core_idx].w_valid    = ace_bus_dv[core_idx].w_valid     ;
   //assign i_culsans.core_to_CCU[core_idx].w_ready    = ace_bus_dv[core_idx].w_ready     ;
   assign i_culsans.core_to_CCU[core_idx].w_last     = ace_bus_dv[core_idx].w_last      ;
                                                                                        
   //assign i_culsans.core_to_CCU[core_idx].b_id       = ace_bus_dv[core_idx].b_id        ;
   //assign i_culsans.core_to_CCU[core_idx].b_resp     = ace_bus_dv[core_idx].b_resp      ;
   //assign i_culsans.core_to_CCU[core_idx].b_user     = ace_bus_dv[core_idx].b_user      ;
   //assign i_culsans.core_to_CCU[core_idx].b_valid    = ace_bus_dv[core_idx].b_valid     ;
   //assign i_culsans.core_to_CCU[core_idx].b_ready    = ace_bus_dv[core_idx].b_ready     ;
