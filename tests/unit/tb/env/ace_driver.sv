...
class ace_driver extends uvm_driver #(ace_txn_item) ;
 ...
 ace_txn_item outstanding_xtn[int];

  mailbox #(ace_txn_item) aw_mb;
  mailbox #(ace_txn_item) w_mb;
  mailbox #(ace_txn_item) ar_mb;
  mailbox #(ace_txn_item) r_mb;
  ...
  `uvm_info("ACE DRIVER", $sformatf("RECEIVED ace_var: c=%0d", c), UVM_MEDIUM)
...
task run_phase(uvm_phase phase); 
   forever begin
    ...
    `uvm_info(get_type_name(), "..........run_phase_started..................", UVM_MEDIUM)
    reset();
    #500;
...
task txn_collector (ace_txn_item xtn);

if (xtn.is_write) begin
    aw_mb.put(xtn);
   `uvm_info(get_type_name(), $sformatf("size_of-aw_mailbox=0x%0d",  aw_mb.num()), UVM_MEDIUM)
    w_mb.put(xtn);
   `uvm_info(get_type_name(), $sformatf("size_of-w_mailbox=0x%0d",  w_mb.num()), UVM_MEDIUM)
    fork
      drive_write_address();
      drive_write_data();
    ...
    ar_mb.put(xtn);
    r_mb.put(xtn);
   `uvm_info(get_type_name(), $sformatf("size_of-ar_mailbox=0x%0d",  ar_mb.num()), UVM_MEDIUM)
   fork 
      drive_read_address();
      receive_read_data();
    join
...
//**************************************************************************//
task drive_write_address();
    `uvm_info(get_type_name(), "START OF WRITE ADDRESS CHANNEL_1", UVM_MEDIUM)
   `uvm_info(get_type_name(), $sformatf("size_of-aw_mailbox=0x%0d",  aw_mb.num()), UVM_MEDIUM)
  ...
  aw_mb.get(xtn);
   ...
   `uvm_info(get_type_name(), $sformatf("size_of-aw_mailbox=0x%0d",  aw_mb.num()), UVM_MEDIUM)
    `uvm_info(get_type_name(), "START OF WRITE ADDRESS CHANNEL_2", UVM_MEDIUM)
   `uvm_info(get_type_name(), $sformatf("is_write=0x%0b",  xtn.is_write), UVM_MEDIUM)
     `uvm_info(get_type_name(), $sformatf("AWADDR=0x%0h, AWBURST=0x%0h",  xtn.aw_addr, xtn.aw_burst), UVM_MEDIUM)
...
repeat($urandom_range(1,5)) @(posedge vif.clk_i);
        ...
        `uvm_info(get_type_name(), $sformatf("vif.awvalid=0x%0b, vif.awready=0x%0b",  vif.aw_valid, vif.aw_ready), UVM_MEDIUM)
    outstanding_xtn [xtn.aw_id]=xtn;
...
task drive_write_data();
    w_mb.get(xtn);
    @aw_done;
    `uvm_info(get_type_name(), "START OF WRITE DATA CHANNEL", UVM_MEDIUM)
 
    for(int i=0; i <= xtn.aw_len + 1; i++) begin   
    @(posedge vif.clk_i);
     
     vif.w_valid     <=      1'b1        ;
     vif.w_data      <=      xtn.w_data[i]          ;
     vif.w_strb      <=      xtn.w_strb          ;
     vif.w_last      <=      (i == xtn.aw_len + 1)           ;
  
 end

//    foreach (xtn.w_data[i]) begin
//      @(posedge vif.clk_i);
//      vif.w_valid  <= 1'b1;
//      vif.w_data   <= xtn.w_data[i];
//      vif.w_strb   <= xtn.w_strb;
//      vif.w_last   <= (i == xtn.w_data.size() - 1);
//    end

        `uvm_info(get_type_name(), $sformatf("WDATA=0X%0H,  WSTRB=0x%0h, w_data_size=0x%0d", xtn.w_data,  xtn.w_strb, xtn.w_data.size()), UVM_MEDIUM)
    do begin
      @(posedge vif.clk_i);
    end while (vif.w_ready !== 1);
...
task handle_write_response();
    ...
    $display("inside handle_write_response — before b_valid");
  @w_done;
    
    vif.b_ready <= 1'b1;   // ← assert b_ready
    #8;
   ...
   while (vif.b_valid !== 1) begin
    ...
    $display("inside handle_write_response — got b_valid");
...
task drive_read_address();
 ...
 //@write_done; 
   `uvm_info(get_type_name(), $sformatf("inside_drive_read_address_size_of-ar_mailbox=0x%0d",  ar_mb.num()), UVM_MEDIUM)
   ...
   `uvm_info(get_type_name(), "START OF READ ADDRESS CHANNEL", UVM_MEDIUM)
   
   `uvm_info(get_type_name(), $sformatf("is_write=0x%0b",  xtn.is_write), UVM_MEDIUM)
 
   if (!xtn.is_write) begin
    @(posedge vif.clk_i);
        vif.ar_addr     <=  xtn.ar_addr     ;
        vif.ar_id       <=  xtn.ar_id       ;
        vif.ar_len      <=  xtn.ar_len      ;
        vif.ar_size     <=  xtn.ar_size     ;
        vif.ar_burst    <=  xtn.ar_burst    ;
        vif.ar_valid    <=  xtn.ar_valid    ;
        vif.ar_snoop    <=  xtn.ar_snoop    ;
        vif.ar_bar      <=  xtn.ar_bar      ;
        vif.ar_domain   <=  xtn.ar_domain   ;
        vif.ar_valid   <= 1'b1;
       
    #8;
   while (!(vif.ar_valid && vif.ar_ready)) begin
    @(posedge vif.clk_i)
    #8;
    end
      @(posedge vif.clk_i);
...
repeat($urandom_range(1,5)) @(posedge vif.clk_i);
        ...
        `uvm_info(get_type_name(), $sformatf("ARADDR=0x%0h, ARBURST=0x%0h",  xtn.ar_addr, xtn.ar_burst), UVM_MEDIUM)
...
task receive_read_data();
   ...
   int beat_cnt;
  `uvm_info(get_type_name(), "START OF READ DATA", UVM_MEDIUM)
   @ar_done;
   vif.r_ready     <=  1     ;
   beat_cnt = xtn.ar_len + 1;
  
   repeat (beat_cnt) begin
  
      while (!vif.r_valid) begin
      @(posedge vif.clk_i);
      end
      @(posedge vif.clk_i);
      ...
      `uvm_info(get_type_name(), $sformatf("RECEIVED_READ_DATA_IS = 0x%0h", vif.r_data), UVM_MEDIUM)
    end
...
task drive_read_address();
    ace_txn_item local_xtn;
    ar_mb.get(local_xtn);
    
    `uvm_info(get_type_name(), "START OF READ ADDRESS CHANNEL", UVM_MEDIUM)
    
    @(posedge vif.clk_i);
    vif.ar_addr    <= local_xtn.ar_addr;
    vif.ar_id      <= local_xtn.ar_id;
    vif.ar_len     <= local_xtn.ar_len;
    vif.ar_size    <= local_xtn.ar_size;
    vif.ar_burst   <= local_xtn.ar_burst;
    vif.ar_snoop   <= local_xtn.ar_snoop;
    vif.ar_domain  <= local_xtn.ar_domain;
    vif.ar_bar     <= local_xtn.ar_bar;
    vif.ar_valid   <= 1'b1;
    
    // Wait for ar_ready handshake
   
    `uvm_info(get_type_name(),
             $sformatf("ARADDR=0x%0h ARBURST=0x%0h", local_xtn.ar_addr, local_xtn.ar_burst),  UVM_MEDIUM)
   ...
   while (!(vif.ar_valid && vif.ar_ready)) begin
    ...
    @(posedge vif.clk_i);
    vif.ar_valid <= 1'b0;  // Deassert after handshake
...
task receive_read_data();
    ...
    r_mb.get(local_xtn);  // or use ar_done event
    beat_count = local_xtn.ar_len +1;
    
    `uvm_info(get_type_name(), "START OF READ DATA CHANNEL_2", UVM_MEDIUM)
    
    // Assert r_ready BEFORE first r_valid can appear
    @(posedge vif.clk_i);
    vif.r_ready <= 1'b1;
    
    // Beat-by-beat handshake — stay ready until r_last
    begin : read_data_loop
        int beat_count = 0;
        
        forever begin
            @(posedge vif.clk_i);
            #1; // small delta to sample stable values
            
            if (vif.r_valid === 1'b1 && vif.r_ready === 1'b1) begin
                // Capture data for this beat
                local_xtn.r_data[beat_count] = vif.r_data;
                local_xtn.r_resp[beat_count] = vif.r_resp;
                
                `uvm_info(get_type_name(), 
                    $sformatf("READ BEAT[%0d]: RDATA=0x%0h, RRESP=0x%0h, RLAST=%0b",
                    beat_count, vif.r_data, vif.r_resp, vif.r_last), UVM_MEDIUM)
                
                beat_count++;
                
                // Exit on last beat
                if (vif.r_last === 1'b1) begin
                    break;
                end
            end
        end
    ...
    // Deassert r_ready only AFTER r_last handshake
    @(posedge vif.clk_i);
    vif.r_ready <= 1'b0;
    
    `uvm_info(get_type_name(), 
        $sformatf("READ COMPLETE: %0d beats received", local_xtn.ar_len + 1), UVM_MEDIUM)
