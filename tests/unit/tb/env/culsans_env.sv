   ...
   //ace_cov_monitor           cov_mon;  
    ...
    function void build_phase(uvm_phase phase); 
      ...
      // uvm_config_db#(uvm_event)::set(this, "*", "finish_event", finish_event);
        ...
        if(!uvm_config_db #(ace_env_config)::get(this, "*", "ace_env_config", ace_env_cfg)) begin
            `uvm_fatal("NO_ACE_env_cfg", "ACE_env_config not found!");
        end else begin
            `uvm_info("ACE_ENV_CONFIG", $sformatf("---------%s[%0d]--------------", "ACE_ENV_CFG", ace_env_cfg.NO_OF_AGENTS), UVM_NONE);  
        end

      if (ace_env_cfg == null) begin
  `uvm_fatal("CFG_NULL", "ace_env_cfg is NULL before agent creation")
end


       
ace_agt = new[ace_env_cfg.NO_OF_AGENTS];
for (int k = 0; k < ace_env_cfg.NO_OF_AGENTS; k++) begin
  ace_agt[k] = ace_agent::type_id::create($sformatf("ace_agt[%0d]", k), this);
  uvm_config_db#(int)::set(this,$sformatf("ace_agt[%0d].*", k),"ace_var", k);
end
...
//      for (int k=0; k < ace_env_cfg.NO_OF_AGENTS; k++) begin
//        int ace_var;
//       uvm_config_db #(int)::set(this, "*",  ace_var, k);
//       ace_agt[k] = ace_agent::type_id::create($sformatf("ace_agt[%0d]", k), this);
//      end
...
*/
 ...
 //       ace_agt[c] = ace_agent::type_id::create($sformatf("ace_agt[%0d]", c), this);
            ...
            if (!uvm_config_db#(virtual SNOOP_BUS_DV#(64,64))::get(this, "*", $sformatf("snoop_bus_dv_vif_%0d", c), snoop_bus_dv[c])) 
