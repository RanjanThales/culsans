...
class cache_line_rw_colision extends culsans_base_test;
  `uvm_component_utils(cache_line_rw_colision)
...
cache_rwc_w11_seq      c_coll_w11_seq    ;
  ...
  function new(string name = "cache_line_rw_colision", uvm_component parent = null);
...
for (int c = 0; c < c_env_cfg.N_C; c++) begin
  ...
  task run_phase(uvm_phase phase);
    ...
    `uvm_info("TESTNAME", $sformatf("Inside cache_line_rw_colision: testname = %s", testname), UVM_LOW)
